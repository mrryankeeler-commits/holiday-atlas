# Incomplete location master queue (14-batch plan)

- Queue basis: locations flagged as short-form enrichment targets from the April 17, 2026 content audit (excluding already fully-complete entries).
- Batch sizing: 13 batches of 3 locations, final batch of 2 locations.

## Batch plan

### Batch 1
- `a-lat-vietnam`
- `arequipa-peru`
- `asuncion-paraguay`

### Batch 2
- `bacalar-mexico`
- `barranquilla-colombia`
- `belize-city-belize`

### Batch 3
- `bishkek-kyrgyzstan`
- `cartagena-colombia`
- `el-cotillo-fuerteventura-spain`

### Batch 4
- `havana-cuba`
- `hegra-saudi-arabia`
- `hoi-an-vietnam`

### Batch 5
- `kampot-cambodia`
- `kutaisi-georgia`
- `leh-kashmir-india`

### Batch 6
- `lima-peru`
- `luang-prabang-laos`
- `madeira-portugal`

### Batch 7
- `makassar-indonesia`
- `malay-philippines`
- `manado-indonesia`

### Batch 8
- `medellin-colombia`
- `minca-colombia`
- `ometepe-nicaragua`

### Batch 9
- `phoenix-arizona-united-states`
- `placencia-belize`
- `puerto-ayora-ecuador`

### Batch 10
- `puno-peru`
- `quetzaltenango-guatemala`
- `quito-ecuador`

### Batch 11
- `reckong-peo-india`
- `rio-de-janeiro-brazil`
- `ronda-spain`

### Batch 12
- `santa-marta-colombia`
- `tangalla-sri-lanka`
- `tekapo-new-zealand`

### Batch 13
- `tilcara-argentina`
- `tokyo-japan`
- `ulaanbaatar-mongolia`

### Batch 14
- `vinales-cuba`
- `waigeo-indonesia`

## Status tracker

| id | coords fixed | content enriched | months complete | qa checked | merged |
|---|---|---|---|---|---|
| `a-lat-vietnam` | yes | no | yes | no | yes |
| `arequipa-peru` | yes | no | yes | no | yes |
| `asuncion-paraguay` | yes | no | yes | no | yes |
| `bacalar-mexico` | yes | no | yes | no | no |
| `barranquilla-colombia` | yes | no | yes | no | no |
| `belize-city-belize` | yes | no | yes | no | no |
| `bishkek-kyrgyzstan` | yes | no | yes | no | no |
| `cartagena-colombia` | yes | no | yes | no | no |
| `el-cotillo-fuerteventura-spain` | yes | no | yes | no | no |
| `havana-cuba` | yes | no | yes | no | no |
| `hegra-saudi-arabia` | yes | no | yes | no | no |
| `hoi-an-vietnam` | yes | no | yes | no | no |
| `kampot-cambodia` | yes | no | yes | no | no |
| `kutaisi-georgia` | yes | no | yes | no | no |
| `leh-kashmir-india` | yes | yes | yes | yes | no |
| `lima-peru` | yes | yes | yes | yes | no |
| `luang-prabang-laos` | yes | no | yes | no | no |
| `madeira-portugal` | yes | no | yes | no | no |
| `makassar-indonesia` | yes | no | yes | no | no |
| `malay-philippines` | yes | no | yes | no | no |
| `manado-indonesia` | yes | no | yes | no | no |
| `medellin-colombia` | yes | no | yes | no | no |
| `minca-colombia` | yes | no | yes | no | no |
| `ometepe-nicaragua` | yes | no | yes | no | no |
| `phoenix-arizona-united-states` | yes | no | yes | no | no |
| `placencia-belize` | yes | no | yes | no | no |
| `puerto-ayora-ecuador` | yes | no | yes | no | no |
| `puno-peru` | yes | yes | yes | yes | no |
| `quetzaltenango-guatemala` | yes | no | yes | no | no |
| `quito-ecuador` | yes | no | yes | no | no |
| `reckong-peo-india` | yes | no | yes | no | no |
| `rio-de-janeiro-brazil` | yes | no | yes | no | no |
| `ronda-spain` | yes | no | yes | no | no |
| `santa-marta-colombia` | yes | no | yes | no | no |
| `tangalla-sri-lanka` | yes | no | yes | no | no |
| `tekapo-new-zealand` | yes | no | yes | no | no |
| `tilcara-argentina` | yes | no | yes | no | no |
| `tokyo-japan` | yes | no | yes | no | no |
| `ulaanbaatar-mongolia` | yes | no | yes | no | no |
| `vinales-cuba` | yes | no | yes | no | no |
| `waigeo-indonesia` | yes | no | yes | no | no |
